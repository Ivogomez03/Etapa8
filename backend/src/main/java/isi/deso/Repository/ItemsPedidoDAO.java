...
public interface ItemsPedidoDAO extends JpaRepository<ItemsPedido, Integer> {
    ...
    // @Query("SELECT ip FROM Vendedor v JOIN Pedido p ON p.id_vendedor =
    // v.id_vendedor JOIN ItemsPedido ip ON ip.id_pedido = p.id_pedido WHERE
    // v.id_vendedor = :idVendedor")
    // public List<ItemsPedido> buscarPorVendedor(int idVendedor);

    // @Query("SELECT ip FROM Cliente c JOIN Pedido p ON p.id_cliente = c.id_cliente
    // JOIN ItemsPedido ip ON ip.id_pedido = p.id_pedido WHERE c.id_cliente =
    // :idCliente")
